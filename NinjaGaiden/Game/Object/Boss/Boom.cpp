...
Boom::Boom() {
	...
	this->sprite
		->insert(pair<ENEMY_STATE, Sprite*>(ENEMY_STATE::FOLLOW,
			new Sprite(TEXTURE->Get(ID_TEXTURE_MAP_1_ENEMY), PATH_TEXTURE_MAP_3_BOOM_BOSS, 1, 0.0f)));
	this->sprite
			...
			if (e->object->GetObjectType() == OBJECT_TYPE::MAIN_CHARACTER) {
				if (!PLAYER->GetWounded()) {
					if (PLAYER->GetState() == JUMP_ATK) {
						...
						PLAYER->Wounded(e->nx, e->ny, this, GetObjectDirection());
					}
				}
			}
			Object::PlusPosition(this->deltaX, this->deltaY);
...
bool Boom::IsOutCamera() {
	return !COLLISION->AABB(GetBoundingBox(), CAMERA->GetRECTx());
}
