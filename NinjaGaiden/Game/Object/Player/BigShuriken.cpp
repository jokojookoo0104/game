...
BigShuriken::BigShuriken() {
	...
	sprite = new Sprite(TEXTURE->Get(BIG_SHURIKEN_ID), PATH_POS_BigShuriken, 2, 0.0475f);
}
...
void BigShuriken::Orbit(float t) {
	left = PLAYER->GetPosition().x + 80.0f;
	right = PLAYER->GetPosition().x - 80.0f;
	if (position.x >= left) {
		SetVx(-400.0f);
	}
	if (position.x <= right) {
	...
	if (PLAYER->GetPosition().y + 4  < position.y) {
