...
Player::Player()
{
	...
	this->sprite
		->insert(pair<PLAYER_STATE, Sprite*>(PLAYER_STATE::STAND,
			new Sprite(TEXTURE->Get(ID_TEXTURE_MAIN), PATH_POS_MAIN_STAND, 1, 0.0f)));
	this->sprite
		->insert(pair<PLAYER_STATE, Sprite*>(PLAYER_STATE::RUN,
			new Sprite(TEXTURE->Get(ID_TEXTURE_MAIN), PATH_POS_MAIN_RUN, 3, 0.055f)));
	this->sprite
		->insert(pair<PLAYER_STATE, Sprite*>(PLAYER_STATE::SIT,
			new Sprite(TEXTURE->Get(ID_TEXTURE_MAIN), PATH_POS_SIT, 1, 0.02)));
	this->sprite
		->insert(pair<PLAYER_STATE, Sprite*>(PLAYER_STATE::SIT_ATK,
			new Sprite(TEXTURE->Get(ID_TEXTURE_MAIN), PATH_POS_SIT_ATK, 3, 0.06f)));
	this->sprite
	...
	this->sprite
		->insert(pair<PLAYER_STATE, Sprite*>(PLAYER_STATE::USE_WEAPON,
			new Sprite(TEXTURE->Get(ID_TEXTURE_MAIN), PATH_POS_WEAPON, 3, 0.07f)));
	this->sprite
		->insert(pair<PLAYER_STATE, Sprite*>(PLAYER_STATE::DIE,
			new Sprite(TEXTURE->Get(ID_TEXTURE_MAIN), PATH_POS_WOUNDED, 1, 0.0f)));
	this->sprite
		->insert(pair<PLAYER_STATE, Sprite*>(PLAYER_STATE::JUMP_ATK,
			new Sprite(TEXTURE->Get(ID_TEXTURE_MAIN), PATH_POS_JUMP_ATK, 4, 0.0475f)));
	this->sprite
		->insert(pair<PLAYER_STATE, Sprite*>(PLAYER_STATE::CLIMB,
			new Sprite(TEXTURE->Get(ID_TEXTURE_MAIN), PATH_POS_CLIMB, 2, 0.09f)));
}
...
void Player::Update(float t, vector<Object*>* object)
{
	...
	if (weapon) {
		...
		if (weapon->GetObjectType() == SMALL_SHURIKEN) {
			for (int i = 0; i < SHURIKEN_SMALL_NUMBER; i++) {
				SmallShuriken* smallshuriken = dynamic_cast<SmallShuriken*>(&weapon[i]);
				smallshuriken->Update(t, object);
			...
			if (weapon->GetObjectType() == BIG_SHURIKEN && COLLISION->AABB(weapon->GetBoundingBox(), GetBoundingBox())) {
				weapon->SetActive(false);
			}
	...
	if (state != PLAYER_STATE::DIE) {
		...
		if (hp == 0 || !COLLISION->AABB(GetBoundingBox(), CAMERA->GetRECTx())) {
...
void Player::Render()
{
	...
	//draw12->DrawRect(katana->GetBoundingBox(), CAMERA);
	// Vẽ WEAPON
	if (weapon) {
		if (weapon->GetObjectType() == CIRCLE_FIRE) {
			for (int i = 0; i < CIRCLE_FIRE_NUMBER; i++) {
		...
		if (weapon->GetActive() == false) {
			...
			case OBJECT_TYPE::SMALL_SHURIKEN:
				if (itemPoint >= MANA_POINT_SHURIKEN_SMALL) {
					MCIPLAYER->Play(SOUND_ATK_SMALL_SHURIKEN);
					...
					for (int i = 0; i < SHURIKEN_SMALL_NUMBER; i++) {
						...
						weapon[i].SetVx(vx*(i+1));
						weapon[i].SetPosition(position.x + posX, position.y );
					}
			...
			case OBJECT_TYPE::BIG_SHURIKEN:
				if (itemPoint >= MANA_POINT_SHURIKEN_BIG) {
					MCIPLAYER->Play(SOUND_ATK_BIG_SHURIKEN);
					if (this->direction == LEFT) {
...
bool Player::Wounded(float nx, float ny, Object* object, DIRECTION direction) {
	...
	MCIPLAYER->Play(SOUND_WOUNDED);
	return false;
}

void Player::KillEnemy(Object* object) {
	if (COLLISION->AABB(katana->GetBoundingBox(), object->GetBoundingBox())) {
		object->Dead();
		return;
	}
	if (weapon) {
		if (weapon->GetObjectType() != CIRCLE_FIRE) {
			if (COLLISION->AABB(weapon->GetBoundingBox(), object->GetBoundingBox())) {
				object->Dead();
			...
			for (int i = 0; i < 3; i++) {
				if (COLLISION->AABB(weapon[i].GetBoundingBox(), object->GetBoundingBox())) {
					object->Dead();
					break;
				}
			}
