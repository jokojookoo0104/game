...
BansheeSword::BansheeSword(int positionX, int positionY, DIRECTION direction) {
	this->SetObjectType(BANSHEESWORD);
	...
	this->sprite
		->insert(pair<ENEMY_STATE, Sprite*>(ENEMY_STATE::FLY,
			new Sprite(TEXTURE->Get(ID_TEXTURE_MAP_1_ENEMY), PATH_TEXTURE_MAP_1_ENEMY_BANSHEE_SWORD, 2, 0.09f)));
	this->sprite
...
void BansheeSword::Update(float t, vector<Object*>* objects) {
	if (PLAYER->isFreezeTime() && isActive) {
		SetVeclocity(0, 0);
		Object::Update(t);
...
void BansheeSword::UpdateWitchPosition(D3DXVECTOR3 witchPosition) {
	this->witchPosition = witchPosition;
}

void BansheeSword::HandleCollision(vector<Object*> *object) {
...
void BansheeSword::Fly(int vX, DIRECTION direction) {
	this->state = FLY;
	this->direction = direction;
...
void BansheeSword::SetState(ENEMY_STATE state) {
	this->state = state;
}