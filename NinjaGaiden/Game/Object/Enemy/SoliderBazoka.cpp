...
void SoliderBazoka::InitSpite() {
	...
	this->sprite
		->insert(pair<ENEMY_STATE, Sprite*>(ENEMY_STATE::DO_NOTHING,
			new Sprite(TEXTURE->Get(ID_TEXTURE_MAP_1_ENEMY), PATH_TEXTURE_MAP_1_ENEMY_SOLIDER_BAZOKA_DO_NOT_THING, 1, 0)));
	this->sprite
		->insert(pair<ENEMY_STATE, Sprite*>(ENEMY_STATE::ATK,
			new Sprite(TEXTURE->Get(ID_TEXTURE_MAP_1_ENEMY), PATH_TEXTURE_MAP_1_ENEMY_SOLIDER_BAZOKA_ATK, 2, 0.09f)));
	this->sprite
...
void SoliderBazoka::HandleAttack(float t) {
	if (state != ENEMY_STATE::ATK) {
		if (timerDelayShooting >= SHOOT_TIME_DELAY) {
			MCIPLAYER->Play(SOUND_ENEMY_BAZOKA);
