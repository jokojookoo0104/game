...
void Panther::init() {
	...
	this->sprite
		->insert(pair<ENEMY_STATE, Sprite*>(ENEMY_STATE::FOLLOW,
			new Sprite(TEXTURE->Get(ID_TEXTURE_MAP_1_ENEMY), PATH_TEXTURE_MAP_1_ENEMY_Panther_follow, 2, 0.08f)));
	this->sprite
...
void Panther::UpdateActiveArea(float t) {
	if (state != DEAD) {
		/*if (PLAYER->GetPosition().x >= 813 && PLAYER->GetPosition().x <= 850) {
		...
		if (PLAYER->GetPosition().x >= 1125 && PLAYER->GetPosition().x <= 1410) {
			if (PLAYER->GetPosition().x >= activeArea.at(0)
				&& PLAYER->GetPosition().x <= activeArea.at(1)
				&& PLAYER->GetDirection() == RIGHT
				&& activeArea.at(0) != 0
