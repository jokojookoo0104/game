...
Bullet::Bullet(int positionX, int positionY, BULLET_TYPE type, DIRECTION direction) {
	...
	case GUN:
		sprite->insert(pair<ENEMY_STATE, Sprite*>(ENEMY_STATE::FLY,
			new Sprite(TEXTURE->Get(ID_TEXTURE_MAP_1_ENEMY), PATH_TEXTURE_MAP_1_ENEMY_SOLIDER_GUN_BULLET, 1, 0)));
		break;
	case BAZOKA:
		sprite->insert(pair<ENEMY_STATE, Sprite*>(ENEMY_STATE::FLY,
			new Sprite(TEXTURE->Get(ID_TEXTURE_MAP_1_ENEMY), PATH_TEXTURE_MAP_1_ENEMY_SOLIDER_BAZOKA_BULLET, 1, 0)));
		break;
	default:
		break;
	}
