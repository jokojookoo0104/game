...
void Grid::GetObjectsInCells(Object * object)
{
	...
	RECT camRECTx = CAMERA->GetRECTx();
	for (int i = 1; i < objects->size(); i++) {
		if (!COLLISION->AABB(objects->at(i)->GetBoundingBox(), camRECTx)
	...
	//			if (!Game::AABB(CAMERA->GetRECTx(), listObj->at(k)->GetBoundingBox())) {
	...
	for (int i = 0; i < squares->size(); i++) {
		if (COLLISION->AABB(camREC, squares->at(i)->GetBoundingBox())) {
	...
	for (int i = y1; i < y2; i++) {
		...
		for (int j = x1; j < x2; j++) {
			...
			for (int k = 0; k < listObj->size(); k++) {
				if (COLLISION->AABB(CAMERA->GetRECTx(), listObj->at(k)->GetBoundingBox())) {
...
void Grid::RenderObject() {
	for (int i = 0; i < objects->size(); i++) {
		objects->at(i)->Render();
		//draw1->DrawRect(objects->at(i)->GetBoundingBox(), CAMERA);
	}
	//DrawGrid();
	//draw1->DrawRect(CAMERA->GetRECT(), CAMERA);
	//draw1->DrawRect(PLAYER->GetKatana()->GetBoundingBox(), CAMERA);
}
