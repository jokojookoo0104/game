...
void Game::InitKeyboard(InputDevice* keyHandler)
{
	...
	if (hr != DI_OK)
	{
		OutputDebugStringW(L"[ERROR] DirectInput8Create failed!\n");
		return;
	}
	...
	if (hr != DI_OK)
	{
		OutputDebugStringW(L"[ERROR] DINPUT8::Acquire failed!\n");
		return;
	}

	OutputDebugStringW(L"[INFO] Keyboard has been initialized successfully\n");
}
...
void Game::ProcessKeyboard()
{
	...
	if (FAILED(hr))
	{
		...
		if ((hr == DIERR_INPUTLOST) || (hr == DIERR_NOTACQUIRED))
		{
			...
			if (h == DI_OK)
			{
				OutputDebugStringW(L"[INFO] Keyboard re-acquired!\n");
			}
			else return;
		}
		else
